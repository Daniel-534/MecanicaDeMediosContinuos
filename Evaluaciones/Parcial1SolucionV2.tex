...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small Mecánica de Medios Continuos}
\fancyhead[R]{\small Parcial 1 · \nouppercase{\leftmark}}
...
\begin{center}
    {\small Universidad de Antioquia · Instituto de Física\par}
    \vspace{4pt}
    {\large Mecánica de Medios Continuos\par}
    \vspace{6pt}
    {\LARGE\bfseries Parcial 1\par}
    \vspace{4pt}
    {8 de septiembre de 2026\par}
\end{center}

\markboth{Enunciados}{Enunciados}
\section*{Enunciados}

\textbf{Indicaciones.} Presente todos los procedimientos matemáticos y exprese
las respuestas de forma simplificada y justificada. Los tres problemas tienen
ponderaciones de 40\%, 30\% y 30\%, respectivamente.

\section{Atmósfera de dos capas\hfill 40\%}

Considere una atmósfera en equilibrio hidrostático sometida a un campo gravitacional constante $\vec{g} = -g_0 \hat{k}$. El aire satisface la ecuación de estado del gas ideal
\[
p = \rho R_s T,
\]
...
\begin{enumerate}[label=\alph*), leftmargin=*]
    \item Obtenga los perfiles de presión $p(z)$ y temperatura $T(z)$ en la capa inferior en términos de $p_0$, $T_0$, $R_s$, $g_0$ y $\gamma$. Determine sus valores en la tropopausa, $p_t = p(z_t)$ y $T_t = T(z_t)$.
    \item Obtenga el perfil de presión $p(z)$ en la región isotérmica, imponiendo la continuidad en $z = z_t$.
    \item Sustituya los valores de $T_0$, $p_0$, $z_t$, $R_s$, $\gamma$ y $g_0$ en los resultados anteriores y evalúe la presión a una altura $z = 20\,\mathrm{km}$.
\end{enumerate}

\clearpage

\section{Esfera autogravitante\hfill 30\%}

Considere una esfera de fluido en equilibrio hidrostático bajo su propia gravedad, con radio $R$ y distribución de densidad
\[
\rho(r) =
\begin{cases}
...
\begin{enumerate}[label=\alph*), leftmargin=*]
    \item Determine la distribución de presión $p(r)$ en el interior de la esfera, imponiendo la condición de frontera $p(R) = 0$.
    \item Calcule la presión central $p_c = p(0)$.
\end{enumerate}

\section{Esfera politrópica\hfill 30\%}

Considere una esfera de fluido autogravitante en equilibrio hidrostático, descrita por la ecuación de estado
\[
p = K\rho^2,
\]
...
\begin{enumerate}[label=\alph*), leftmargin=*]
    \item A partir de las ecuaciones de equilibrio hidrostático y de conservación de masa, demuestre que
    \[
    \frac{1}{\xi^2}\frac{d}{d\xi}
    \left(\xi^2\frac{d\theta}{d\xi}\right) = -\theta.
    \]
    \item Resuelva esta ecuación suponiendo una solución regular en el centro y normalizada mediante $\theta(0) = 1$.
    \item Determine el radio de la esfera, definido por el primer cero positivo de $\theta$, el perfil de densidad $\rho(r)$ y la masa total.
\end{enumerate}

\clearpage
\setcounter{section}{0}
\markboth{Soluciones}{Soluciones}
\begin{center}
    {\LARGE\bfseries Soluciones\par}
    \vspace{3pt}
    {\small Desarrollo por problema e inciso\par}
\end{center}

\section{Atmósfera de dos capas}
...
\subsection{Perfiles en la capa inferior y valores en la tropopausa}
...
\Needspace{7\baselineskip}
\subsection{Perfil en la región isotérmica}
...
\Needspace{7\baselineskip}
\subsection{Evaluación a una altura de 20 km}

\textit{Pendiente de datos:} el enunciado no especifica los valores numéricos de
$T_0$, $p_0$, $z_t$, $R_s$, $\gamma$ y $g_0$. La evaluación numérica requiere
completar estos datos y utilizar el perfil correspondiente a la altura indicada.


\clearpage

\section{Esfera autogravitante}

\Needspace{7\baselineskip}
\subsection{Distribución de presión en el interior}
...
\[
\boxed{p_c = \frac{5\pi G \rho_c^2 R^2}{36}}.
\]

\clearpage

\section{Esfera politrópica}
...
\Needspace{7\baselineskip}
\subsection{Ecuación adimensional}
...
\Needspace{7\baselineskip}
\subsection{Solución regular en el centro}
...
\Needspace{7\baselineskip}
\subsection{Radio, densidad y masa total}

\subsubsection*{Radio de la esfera}
...
\subsubsection*{Perfil de densidad}
...
\boxed{
\begin{aligned}
    \theta(\xi) &= \frac{\sin\xi}{\xi}, \qquad \theta(0)=1,\\[4pt]
    R &= \pi\left(\frac{K}{2\pi G}\right)^{1/2},\\[4pt]
    \rho(r) &= \rho_c\frac{\sin(\pi r/R)}{\pi r/R},
        \qquad 0<r\le R,\\[4pt]
    \rho(0) &= \rho_c, \qquad \rho(r)=0 \quad (r>R),\\[4pt]
    M &= 4\pi^{2}\rho_c a^{3}
