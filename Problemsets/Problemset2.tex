...
\section{Problema 5}
En la figura se muestra una compuerta rectangular de ancho $w$ que puede rotar en torno al punto $C$ y hace un ángulo $\alpha$ con el suelo. Calcule la fuerza $\mathcal{F}$ necesaria para mantener la compuerta en posición. Ignore la presión atmosférica.
...
Se elige una coordenada $\ell$ medida a lo largo de la compuerta, comenzando en la superficie libre y aumentando hacia la articulación $C$. Por tanto, $\ell=0$ en la superficie y $\ell=L$ en $C$. La profundidad vertical de un punto situado a una distancia $\ell$ es
$$
h(\ell)=\ell\sin\alpha.
$$
...
$$
F_{\text{agua}}
=\int_0^L \rho g\sin\alpha\,\ell w\,d\ell
=\frac{\rho g\,wH^2}{2\sin\alpha}.
$$

\subsubsection{Equilibrio de torques respecto a \texorpdfstring{$C$}{C}}
...
\input{FigurasPS2/Fig6.tex}

Cada fuerza diferencial $dF$ actúa perpendicularmente a la compuerta. Como $\ell$ se mide desde la parte superior, la distancia entre el punto de aplicación de $dF$ y la articulación $C$ es
$$
L-\ell.
$$
...
\tau_{C,\text{agua}}
&=\rho g w\sin\alpha
\left(L\frac{L^2}{2}-\frac{L^3}{3}\right)\\
&=\rho g w\sin\alpha\,L^3
\left(\frac{1}{2}-\frac{1}{3}\right)\\
&=\frac{\rho g w\sin\alpha\,L^3}{6}.
\end{aligned}
$$
...
\mathcal{F}L-\tau_{C,\text{agua}}=0.
$$

Por tanto,
$$
\mathcal{F}=\frac{\tau_{C,\text{agua}}}{L}
=\frac{\rho g w\sin\alpha\,L^2}{6}.
$$
...
La componente horizontal de la fuerza hidrostática sobre una superficie curva es igual a la fuerza ejercida sobre su proyección vertical. Como no se conoce la longitud axial del cilindro, se considera una porción arbitraria de longitud $\ell$. Su proyección vertical es entonces un rectángulo de altura $2R$ y ancho $\ell$, cuyo elemento de área es $dA=\ell\,dy$. La presión varía linealmente desde cero en la superficie libre ($y=0$) hasta $2\rho gR$ en el fondo ($y=2R$).

\input{FigurasPS2/Fig8.tex}

Sobre la mitad izquierda del cilindro, la presión del agua empuja en dirección positiva del eje $x$. Por esta razón, la componente horizontal de la fuerza del agua sobre la porción de longitud $\ell$ se toma positiva:
$$
F_{h,x}=\int_A p(y)\,dA
=\ell\int_0^{2R}\rho gy\,dy
=2\rho gR^2\ell.
$$

Al dividir entre la longitud axial considerada, se obtiene la fuerza horizontal por unidad de longitud:
$$
\frac{F_{h,x}}{\ell}=2\rho gR^2>0.
$$
...
\sum F_x=0
\quad\Longrightarrow\quad
\frac{F_x}{\ell}+\frac{F_{h,x}}{\ell}=0.
$$

Por tanto, la fuerza externa por unidad de longitud es:
$$
\boxed{\frac{F_x}{\ell}=-2\rho gR^2}
$$

El signo negativo indica que la fuerza está dirigida hacia la izquierda. Su magnitud por unidad de longitud es $\boxed{F/\ell=2\rho gR^2}$.
